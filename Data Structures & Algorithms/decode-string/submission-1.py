class Solution:
    def decodeString(self, s: str) -> str:
        ...
        # To convert the recursive method to an interative one,
        # we need to save the current string that we are building
        # in a stack and resolve inner most content
        ...
        for c in s:
            if c.isdigit():
                curr_amt = curr_amt * 10 + int(c)
            elif c == "[":
                stack.append((curr, curr_amt))
                curr = ""
                curr_amt = 0
            elif c == "]":
                prev_str, amt = stack.pop()
                curr = prev_str + curr * amt
            else:
                curr += c
